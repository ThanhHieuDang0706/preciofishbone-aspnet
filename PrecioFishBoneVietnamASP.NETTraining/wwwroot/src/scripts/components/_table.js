...
<tr>
  ...
  <td class="table-first-row">
    ...
    <div class="col-xs-9">
      ${!isFile(item) ? folderIcon : mapFileExtensionToIcon(item.fileExtension)}
    </div>
  </td>
  ...
  <td class="f-name" data-id="${!isFile(item) ? item.id : ''}" data-type="${isFile(item) ? 'file' : 'folder'}" >
    ...
    <div class="col-xs-9">
      ${!isFile(item) ? item.name : `${item.name}${item.fileExtension}`}
    </div>
  </td>
  ...
  <td>
    ...
    <div class="modified-col col-xs-9">
      <div class="btn-group" role="group" aria-label="btn actions">
        <button data-toggle="modal" data-target="#modal-form" data-action="edit" data-id="${item.id}" data-type="${isFile(item) ? 'file' : 'folder'}" type="button" class="btn btn-sm btn-primary">
...
export const renderTable = async (state) => {
    ...
    table.empty();
    if (items.length === 0)
        return table.append('<p class="text-center">There is no item in this folder</p>');
    // add table header
